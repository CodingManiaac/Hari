...
import qualitiesData from "@/chapters/chapter-10/content/qualities.json";
...
// Sparkles and hearts canvas for popup details
function SparkleHeartsCanvas() {
  ...
  useEffect(() => {
    ...
    const spawnParticle = (isInitial = false) => {
      ...
      return {
        ...
        size: type === "heart" ? Math.random() * 6 + 4 : Math.random() * 4 + 2,
        type,
        speedY: -(Math.random() * 1.2 + 0.6), // float up
        speedX: Math.random() * 0.6 - 0.3,
        opacity: Math.random() * 0.5 + 0.3,
        fadeSpeed: Math.random() * 0.001 + 0.0008,
    ...
    const animate = () => {
      ...
      particles.forEach((p) => {
        ...
        if (p.type === "heart") {
          ...
          ctx.fillStyle = `rgba(253, 224, 71, ${p.opacity})`; // gold sparkles
          ctx.fill();
        }
      });
...
export default function Chapter10View() {
  const [selectedQuality, setSelectedQuality] = useState<Quality | null>(null);

  // Soft pastel gradients for scrapbook feeling
  const cardStyles = [
  ...
  return (
    <ChapterContainer
      ...
      subtitle="A collection of the beautiful qualities, moments of strength, and caring details that make you so incredibly special."
    >
      <div className="relative w-full py-8 space-y-12 select-none flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-5xl w-full px-6 z-10">
          {qualitiesData.map((q: Quality, idx: number) => {
              ...
              <div className="w-full scrapbook-page border-glow-pink shadow-2xl rounded-3xl p-6 md:p-8 relative overflow-hidden flex flex-col justify-between items-center text-center space-y-6">
                ...
                <div className="space-y-3">
                  <span className="text-[10px] font-sans font-bold text-primary tracking-widest uppercase block flex justify-center items-center gap-1">
                    <Sparkles className="w-3.5 h-3.5 text-primary" /> SIBLING APPRECIATION
                  </span>
                  <h3 className="text-xl md:text-2xl font-display font-extrabold text-pink-700 leading-snug">
                    {selectedQuality.title}
                  </h3>
                  <p className="text-sm md:text-base text-rose-950/80 leading-relaxed font-sans bg-white/30 border border-pink-50/50 p-4 rounded-xl shadow-inner font-medium">
                ...
                <div className="flex items-center gap-1.5 text-[10px] text-rose-300 font-bold font-sans uppercase tracking-widest border-t border-pink-100/50 w-full pt-4 justify-center">
                  <Heart className="w-4 h-4 fill-primary text-primary" /> Forever Grateful
                </div>
